...
class DirectTemporalNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_times=1, output_ch=4, skips=[4],
                 use_viewdirs=False, embed_fn=None, zero_canonical=True):
        ...
        self.NeRF = NeRF(D=D, W=W, input_ch=input_ch, input_ch_views=input_ch_views, output_ch=output_ch, skips=skips,
                                use_viewdirs=use_viewdirs)
        self.time_linear = nn.ModuleList(
            [nn.Linear(input_ch + input_ch_times, W)] + [nn.Linear(W, W) if i not in skips else nn.Linear(W + input_ch, W) for i in range(D-1)])
        
        self.time_out_layer = nn.Linear(W, 3)

    def query_time(self, new_pts, t):
        h = torch.cat([new_pts, t], dim=-1)
        for i, l in enumerate(self.time_linear):
            h = self.time_linear[i](h)
            h = F.relu(h)
            if i in self.skips:
                h = torch.cat([new_pts, h], -1)

        return self.time_out_layer(h)

    def forward(self, x):
        input_pts, input_views, input_times = torch.split(x, [self.input_ch, self.input_ch_views, self.input_ch_times], dim=-1)

        cur_time = input_times[0, 0]
        ...
        x = torch.cat([input_pts, input_views], dim=-1)
        outputs = self.NeRF(x)
        return outputs
