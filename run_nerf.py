...
def create_nerf(args, near, far):
    ...
    embedtimes_fn, input_ch_times = get_embedder(args.multires, 1, args.i_embed)
    ...
    model = DirectTemporalNeRF(D=args.netdepth, W=args.netwidth,
                 ...
                 embed_fn=embed_fn, zero_canonical=args.zero_canonical
                 ).to(device)
    grad_vars = list(model.parameters())

    model_fine = None
    if args.N_importance > 0:
        model_fine = DirectTemporalNeRF(D=args.netdepth, W=args.netwidth,
                 input_ch=input_ch, output_ch=output_ch, skips=skips,
                 input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs,
                 input_ch_times=input_ch_times,
                 embed_fn=embed_fn, zero_canonical=args.zero_canonical
    ...
    # network_fn 就是 model
    network_query_fn = lambda inputs, viewdirs, frame_times, network_fn : run_network(inputs, viewdirs, frame_times, network_fn,
                                                                embed_fn=embed_fn,
                                                                embeddirs_fn=embeddirs_fn,
                                                                embedtime_fn=embedtimes_fn,
...
def run_network(inputs, viewdirs, frame_times, fn, embed_fn, embeddirs_fn, embedtime_fn, netchunk=1024*64):
    """
    Arguments:
    ...
    for i in range(0, N_rays, chunk):
        ...
        frame_times_chunk = frame_times[i:next_i]                                               # (chunk, 1)
        ...
        if frame_times_chunk is not None:
            input_frame_time = frame_times_chunk[:, None].expand([chunk_rays, N_points_per_ray, 1])     # (chunk, N_points_per_ra, 1)
            input_frame_time_flat = torch.reshape(input_frame_time, [-1, 1])                            # (chunk * N_points_per_ray, 1)
            embedded_time = embedtime_fn(input_frame_time_flat)                                         # (chunk * N_points_per_ray, 21)
            embedded = torch.cat([embedded, embedded_time], -1)                                         # (chunk * N_points_per_ray, 112)
...
def render_path(hwf, K, chunk, render_poses, render_times, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    '''
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ret = render(H, W, K, chunk, c2w=c2w[:3,:4], frame_time=render_times[i], **render_kwargs)
...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, c2w_staticcam=None, frame_time=None,
    ...
    frame_times = frame_time * torch.ones_like(rays_d[...,:1])
    rays = torch.cat([rays_o, rays_d, near, far, frame_times], -1)
    if use_viewdirs:
...
def render_rays(
    ...
    near, far, frame_times = bounds[...,0], bounds[...,1], bounds[...,2] # [-1,1]
    ...
    raw = network_query_fn(pts, viewdirs, frame_times, network_fn)
    ...
    if N_importance > 0:
        ...
        raw = network_query_fn(pts, viewdirs, frame_times, run_fn)
...
def config_parser():
    ...
    parser.add_argument("--zero_canonical", action='store_true',
                        help='if set zero time is the canonic space')
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, _ = render_path(hwf, K, args.chunk, render_poses, render_times, render_kwargs_test, gt_imgs=images, savedir=testsavedir, render_factor=args.render_factor)
    ...
    for i in pbar:
        ...
        ret = render(H, W, K, args.chunk, rays=batch_rays, frame_time=frame_time, **render_kwargs_train)
        ...
        if i%args.i_video==0:
            ...
            with torch.no_grad():
                rgbs, disps = render_path(hwf, K, args.chunk, render_poses, render_times, render_kwargs_test)
        ...
        if i%args.i_testset==0:
            ...
            os.makedirs(testsavedir, exist_ok=True)
            with torch.no_grad():
                render_path(hwf, K, args.chunk, poses[i_test], times[i_test], render_kwargs_test, gt_imgs=images[i_test], savedir=testsavedir)
        ...
        if i%args.i_img==0:
            ...
            target = torch.Tensor(images[img_i]).to(device)
            with torch.no_grad():
                ret = render(H, W, K, chunk=args.chunk, c2w=poses[img_i], frame_time=times[img_i], **render_kwargs_test)
